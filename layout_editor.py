...
class LayoutEditor:
    def __init__(self, parent):
        ...
        self.paper_orientation = "Paisagem"
        ...
        self.window.after(500, self.update_pipeline)
    ...
    def create_window(self):
        ...
        self.notebook.bind("<<NotebookTabChanged>>", lambda e: self.window.after(100, self.show_preview))
    ...
    def create_background_tab(self):
        ...
        tk.Button(gf, text="Aplicar", command=self.apply_gradient, bg='#8e44ad', fg='white').pack(side='left', padx=5, fill='x', expand=True)

        self.add_lbl(sidebar, "TEXTURAS")
        # --- TEXTURAS RESTAURADAS ---
        ...
        self.preview_container_bg = tk.Frame(cont, bg='#95a5a6')
        self.preview_container_bg.pack(side='right', fill='both', expand=True, padx=20, pady=20)
        
        self.canvas_bg = tk.Canvas(self.preview_container_bg, bg='white', bd=0, highlightthickness=0)
    ...
    def create_card_tab(self):
        ...
        self.preview_container_card = tk.Frame(frame, bg='#7f8c8d')
        self.preview_container_card.pack(fill='both', expand=True, padx=20, pady=20)
        
        self.canvas_card = tk.Canvas(self.preview_container_card, bg='white', cursor="crosshair", bd=0, highlightthickness=0)
    ...
    def create_config_tab(self):
        ...
        gp = tk.LabelFrame(cont, text="🖨️ Papel e Impressão", font=('Arial', 12, 'bold'), bg='white', padx=10, pady=10)
        gp.pack(fill='x', pady=5)
        
        tk.Label(gp, text="Tamanho do Papel:", bg='white').pack(anchor='w')
        self.combo_paper = ttk.Combobox(gp, values=list(self.PAPER_SPECS.keys()), state='readonly'); self.combo_paper.pack(fill='x')
        ...
        self.combo_orient = ttk.Combobox(gp, values=["Paisagem", "Retrato"], state='readonly'); self.combo_orient.pack(fill='x')
        self.combo_orient.bind("<<ComboboxSelected>>", self.on_paper_change)
        
        gt = tk.LabelFrame(cont, text="Tempos e Mensagens", font=('Arial', 11, 'bold'), bg='white', padx=10, pady=10); gt.pack(fill='x', pady=5)
        self.create_input(gt, 0, "Início (s):", "spin_start", 5)
        self.create_input(gt, 1, "Intervalo (s):", "spin_interval", 3)
        self.create_input(gt, 2, "Msg Início:", "entry_start", "PREPARE-SE!", True)
        self.create_input(gt, 3, "Msg Fim:", "entry_end", "FOTO SALVA!", True)
    ...
    def reset_canvas_to_size(self):
        ...
        if self.paper_orientation == "Retrato":
            self.real_width = min(w, h); self.real_height = max(w, h)
        else:
            self.real_width = max(w, h); self.real_height = min(w, h)
    ...
    def show_preview(self):
        c_w = self.preview_container_bg.winfo_width()
        c_h = self.preview_container_bg.winfo_height()
        if c_w < 50: c_w = int(self.screen_w * 0.7)
        if c_h < 50: c_h = int(self.screen_h * 0.7)
        
        scale = min(c_w / self.real_width, c_h / self.real_height) * 0.95
    ...
    # --- LÓGICA DE MOUSE E SLOTS (Inalterada) ---
    ...
    def reset_canvas_to_size_button(self): self.reset_canvas_to_size()
    ...
    def save_all(self):
        try:
            ...
            d = {
                "slots": self.slots, 
                "countdown_start": int(self.spin_start.get()), 
                "countdown_interval": int(self.spin_interval.get()),
